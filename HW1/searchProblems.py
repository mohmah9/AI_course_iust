...
class LineByLineProblem (SearchProblem):
    ...
    def __init__(self, startingGameState, goal=(1, 1) ,costFn=lambda x: 1):
        """
        ...
        for i in foods:
            i.reverse()
            fd.append(i)
        fdd=[[row[i] for row in fd] for i in range(len(fd[0]))]
        fdd.reverse()
        for i in range(len(fdd)):
            for j in range(len(fdd[0])):
                if (fdd[i][j]):
                    self.food_position.append((j,i))
        for col in self.food_position:
            if col[0] in self.columns:
                self.columns[col[0]].append(col)
            else:
                self.columns[col[0]]=[col]
        for col in self.columns:
            self.columns[col] = sorted(self.columns[col], key=lambda tup: tup[1])
        for col in self.columns:
            self.detailed_columns[col]=[]
            for i in range (self.columns[col][0][1],self.columns[col][-1][1]+1):
                self.detailed_columns[col].append((col,i))
        for j in self.detailed_columns:
            self.column_num.append(j)
        self.column_num.sort()
        for col in self.column_num:
            if len(self.detailed_columns[col])==1:
                self.traverse_points.append(self.detailed_columns[col][0])
            else:
                self.traverse_points.append(self.detailed_columns[col][-1])
                self.traverse_points.append(self.detailed_columns[col][0])
    ...
    def isGoalState(self, state):
        ...
        if self.pos < len(self.traverse_points):
            if state == self.traverse_points[self.pos]:
                tmp=self.traverse_points[self.pos]
                self.pos+=1
                return state == tmp
            else:
                return state == self.traverse_points[self.pos]
...
class ScaryProblem (PositionSearchProblem):
    ...
    def cost_function(self, state):
        ...
        # print "ghosts__:",self.ghosts
        ghos=self.ghosts
        avoid=[]
        for g in ghos:
            avoid.append(g)
            avoid.append((g[0]+1,g[1]))
            avoid.append((g[0],g[1]+1))
            avoid.append((g[0]-1,g[1]))
            avoid.append((g[0],g[1]-1))
            avoid.append((g[0]+1,g[1]+1))
            avoid.append((g[0]-1,g[1]-1))
            avoid.append((g[0]+1,g[1]-1))
            avoid.append((g[0]-1,g[1]+1))
